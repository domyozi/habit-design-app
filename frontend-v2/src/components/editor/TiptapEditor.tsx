...
import { buildBlockUrl } from '@/lib/note-anchor'
...
const LinkCopiedToast = ({ onDismiss }: { onDismiss: () => void }) => {
  useEffect(() => {
    const t = setTimeout(onDismiss, 1800)
    return () => clearTimeout(t)
  }, [onDismiss])
  return createPortal(
    <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full border border-[#7dd3fc]/30 bg-[#0d1825]/96 px-4 py-2 text-xs text-[#aee5ff] shadow-2xl backdrop-blur-xl">
      🔗 リンクをコピーしました
    </div>,
    document.body,
  )
...
interface Props {
  ...
  /** アンカーリンク URL に埋め込むノート ID。未指定なら Cmd+Shift+A は無効。 */
  noteId?: string
}
...
export function TiptapEditor({ content, onChange, onCharCount, placeholder = 'ここに書き殴ってください... (/ でブロック挿入)', actionsSlot, headerSlot, noteId }: Props) {
  const imageInputRef = useRef<HTMLInputElement>(null)
  const [slash, setSlash] = useState<SlashState | null>(null)
  const [linkCopiedAt, setLinkCopiedAt] = useState<number | null>(null)
  // 最新の noteId をクロージャから参照する用（editorProps はマウント時の値を保持してしまうため）
  const noteIdRef = useRef(noteId)
  useEffect(() => { noteIdRef.current = noteId }, [noteId])

  // 選択中ブロックの id を見つけて URL をクリップボードにコピー。
  // 失敗時 false を返す（キー処理を握り潰さない判断材料に使う）。
  const copyCurrentBlockLink = (state: { selection: { $from: { depth: number; node: (d: number) => { attrs: Record<string, unknown> } } } }): boolean => {
    const noteIdNow = noteIdRef.current
    if (!noteIdNow) return false
    const { $from } = state.selection
    for (let d = $from.depth; d >= 0; d--) {
      const node = $from.node(d)
      const id = node.attrs?.id as string | undefined
      if (id) {
        const url = buildBlockUrl(noteIdNow, id)
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor, transaction }) => {
      // UniqueId 拡張による ID バックフィルだけのトランザクションは保存トリガーから除外する
      // （ノートを開いただけで updated_at が進んでしまう副作用を防ぐ）
    ...
    <div className="relative flex-1 flex flex-col">
      ...
      {slash && (
        <SlashMenu
          ...
          onCopyBlockLink={() => copyCurrentBlockLink(editor.state)}
        />
      )}

      {linkCopiedAt && (
        <LinkCopiedToast key={linkCopiedAt} onDismiss={() => setLinkCopiedAt(null)} />
      )}
