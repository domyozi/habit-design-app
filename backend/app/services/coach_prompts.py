...
現時点で実行可能なactionは primary_target, habit_today_completes, tasks, habits, habit_updates, task_updates, memory_patch のみ。
**削除（habit_delete / task_delete / goal_delete 等）は未許可**。削除提案は出さず、応答文で「不要なら手動で消してください」に留める。
