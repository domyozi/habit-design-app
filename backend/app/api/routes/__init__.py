...
from app.api.routes import admin_eval, ai_coach, coach, daily_logs, goals, google_oauth, habit_suggestions, habits, integrations, journal, kpis, mandala, me, monthly_targets, notes, notes_ai, notifications, ops_tasks, primary_target, tasks, time_windows, todo_definitions, user_context, users, voice_input, wanna_be, workouts
...
# 【表示タイミング (Sprint v6) ルーター】: /api/user/time-windows エンドポイント
api_router.include_router(time_windows.router, tags=["time-windows"])
...
# 【Coach Real backend (Phase 6.5)】: /api/ai/coach-context, /api/ai/coach-stream,
#   /api/ai/coach-pending-actions
...
# 【Tasks ルーター】: /api/tasks エンドポイント（Daily OS v3 Calendar / Flow）
...
# 【Google OAuth ルーター】: /api/integrations/google エンドポイント（Phase 7.3 Calendar 連携）
api_router.include_router(google_oauth.router, tags=["google-oauth"])

# 【ノートルーター】: /api/notes エンドポイント
api_router.include_router(notes.router, tags=["notes"])

# 【Notes inline AI】: /api/ai/notes-prompt-stream エンドポイント (Phase A)
api_router.include_router(notes_ai.router, tags=["notes-ai"])

# 【習慣候補ルーター】: /api/habit-suggestions エンドポイント
api_router.include_router(habit_suggestions.router, tags=["habit-suggestions"])

# 【Sprint coach-eval Phase B】: /api/admin/eval/* (LLM-as-judge 評価結果)
api_router.include_router(admin_eval.router, tags=["admin-eval"])

# 【BODY タブ Sprint body-v1】: /api/workouts/* (筋トレ / ランニング セッション)
